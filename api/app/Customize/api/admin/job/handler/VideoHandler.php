...
namespace App\Customize\api\admin\job\handler;
...
class VideoHandler
{
    ...
    public function handle()
    {
        try {
            ...
            $video = VideoModelHandler::handle($video);
            // 附加：视频
            VideoModelHandler::videos($video);
            // 附加：字幕
            VideoModelHandler::videoSubtitles($video);
            ...
            if ($video->disk === 'local') {
                ...
                if ($video->disk === 'aliyun') {
                    $this->aliyunCloudHandle();
                } else {
                    // todo 其他预留
                }
                if (File::isEmpty($save_dir)) {
                    // 删除空文件夹
    ...
    public function aliyunCloudHandle()
    {
        ...
        foreach ($this->video->videos as $v)
        {
            try {
                ...
                if (empty($resource)) {
                    throw new Exception("资源记录未找到：【{$v->src}】");
                }
                if ($resource->disk !== 'aliyun') {
                    // 跳过非本地存储的资源
                    DB::rollBack();
