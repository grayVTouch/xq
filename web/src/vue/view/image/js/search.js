...
export default {
    ...
    methods: {
        ...
        pageEventInImage (page , size) {
            this.dom.html.scrollTop(0);
            this.images.page = page;
            this.images.size = size;
            this.getWithPager();
        } ,

        sizeEventInImage (size , page) {
            this.dom.html.scrollTop(0);
            this.images.size = size;
            this.images.page = page;
            this.getWithPager();
        ...
        consumeDataBySize (data , size = 24) {
            let count = 0;
            const res = [];
        ...
        getWithPager () {
            ...
            Api.image
                ...
                .then((res) => {
                    ...
                    this.images.allData = data.data;
                    // 还得做进一步的切割
                    this.images.data = this.consumeDataBySize(data.data , this.images.chunkSize);
        ...
        lazyLoadEvent () {
            // 滚动高度
            if (this.images.allData.length === 0) {
                return ;
            }
            const y = this.dom.html.scrollTop();
            const clientH = this.dom.html.clientHeight();
            const extraH = 50;
            const viewH = this.dom.view.height();
            const maxH = viewH + TopContext.val.headerH + 20;
            if (clientH + y + extraH < maxH) {
                return ;
            }
            const res = this.consumeDataBySize(this.images.allData , this.images.chunkSize);
            this.images.data = this.images.data.concat(res);
        } ,
        ...
        initEvent () {
            ...
            this.dom.win.on('scroll' , this.lazyLoadEvent.bind(this));
        } ,
    } ,
}
